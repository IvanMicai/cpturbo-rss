...
s.every '5m' do
	...
	while(continue_load)
		page.search('#forum_table tr').each do |post|
			...
			if(Post.where(id: topic['post_id'].to_i).length != 0)
				continue_load = false
				next
			end
		...
		if(page.search('.next a').length == 0)
			continue_load = false
			next
		end

		page = mechanize.get(page.search('.next a')[0].attribute('href').value)
	end
